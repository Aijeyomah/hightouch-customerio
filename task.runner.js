...
class TaskRunner {
  ...
  addOrUpsertCustomer(sourceData) {
    ...
    const remainingDataIsLessThanMaxParallelism =
      sourceData.length < this.MAX_PARALLELISM;
    const maxItems = remainingDataIsLessThanMaxParallelism
      ? sourceData.length
    ...
    for (let i = 0; i < maxItems; i++) {
      ...
      customerData.push(
        this.request.put(this.customerURL(data.email), mappedUserData)
      );
    }
  ...
  errorIsRetryAble(statusCode) {
    const retryableCodes = [408, 500, 502, 503, 504, 522, 524];
  ...
  errorIsNotRetryAble(statusCode) {
    return (
      statusCode !== 200 &&
      statusCode !== 201 &&
      !this.errorIsRetryAble(statusCode)
    );
  }

  retryTasks(data, time) {
    const listOfTasksToRetry = [];
    data.forEach(({ reason, value }, i) => {
      ...
      if (this.errorIsNotRetryAble(statusCode)) {
        //  not retryable error
        console.log(
          `Error occurred when upserting task for ${this.data[i].email}`
        );
    ...
    while (this.data.length && noOfTries < 1) {
      try {
        const results = await Promise.allSettled(
          this.addOrUpsertCustomer(this.data)
        );
